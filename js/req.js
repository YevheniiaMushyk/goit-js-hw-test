...
if (list) {
	const storedWorkoutIds = JSON.parse(localStorage.getItem("ENERGY_FLOW_FAVORITES_KEY")) || [];
...
function addWorkoutCardToDOM(workoutData) {
	...
	document.querySelectorAll(".exercises_start").forEach((el) => el.addEventListener("click", openFavExerciseModal));
}
...
function removeWorkoutCardFromDOM(removeButton, workoutId) {
	...
	const storedWorkoutIds = JSON.parse(localStorage.getItem("ENERGY_FLOW_FAVORITES_KEY")) || [];
	const updatedStoredWorkoutIds = storedWorkoutIds.filter((id) => id !== workoutId);

	// Оновлення storedWorkoutIds після видалення
	localStorage.setItem("ENERGY_FLOW_FAVORITES_KEY", JSON.stringify(updatedStoredWorkoutIds));
...
function createWorkoutCardMarkup(workoutData) {
    ...
    <li class="exercises_item" data-gifUrl=${workoutData.gifUrl} data-description="${workoutData.description}" data-equipment=${
  ...
  <button class="exercises-start-button" data-workout-data="${JSON.stringify(workoutData)}">
    ...
    >
      ...
      <div class="exercises-text">
        <p class="exercises-text__content">
          ...
          <span class="exercises_text__dynamic">${workoutData.burnedCalories} / 3 min</span>
        </p>
        <p class="exercises-text__content">
      ...
      <span class="exercises_rating__text disactive_video_window">${String(workoutData.rating).padEnd(3, ".0")}</span>
    </li>
  `;
...
export function updateWorkoutCardInFavorites() {
	// isFirstLoad = true;
	const storedWorkoutIdsFav = JSON.parse(localStorage.getItem("ENERGY_FLOW_FAVORITES_KEY")) || [];
